...
# All knee channels: 4,5,6,7  |  All ankle channels: 8,9,10,11
# Updated after rebuild — knee/ankle servo rotation direction flipped on all legs
INVERTED_CHANNELS = {4, 5, 6, 7, 8, 9, 10, 11}

# Left-side legs need hip direction mirrored vs right-side legs
# so that all legs move forward/backward together physically.
# BL=0, FL=3 are left side — their hip servo sweeps opposite to BR=1, FR=2
HIP_INVERTED_LEGS = {BL, FL}

def apply_inversion(ch, angle):
    """Mechanical inversion for knee/ankle channels flipped during rebuild."""
    return (180 - angle) if ch in INVERTED_CHANNELS else angle

def apply_hip_mirror(leg, angle):
    """Mirror hip direction for left-side legs so all legs push same direction."""
    return (180 - angle) if leg in HIP_INVERTED_LEGS else angle

def output_angle(leg, joint, angle):
    """
    Single unified function — always use this before sending any servo command.
    Applies hip mirroring (left-side legs) then mechanical inversion (knee/ankle).
    All paths — sliders, poses, gait engine — must go through here.
    """
    ch = servo_ch(leg, joint)
    if joint == 0:
        angle = apply_hip_mirror(leg, angle)
    return apply_inversion(ch, angle)
...
class GaitEngine:
    ...
    # ── Balance stance ────────────────────────────────────────────────────────
    # Holds all legs at stance and continuously nudges knee angles
    # based on IMU body_tilt — no walking, pure static balance test.
    # IMU_ON must be active and auto_tilt checkbox enabled in IMU tab.
    # Call start_balance() / stop_balance() from UI.
    ...
    def start_balance(self):
        ...
        self._thread.start()
        self.log("⚖️  Balance stance started — enable IMU stream + auto tilt")

    def _balance_loop(self):
        """Hold stance, poll body_tilt every 100ms, adjust knees live."""
        ...
        while self._running:
            ...
            for leg in range(4):
                ...
                self._send(leg, 2, ankle)
            time.sleep(0.1)  # 10Hz correction loop matches IMU stream rate
        self._stance_all()
...
class SerialManager:
    """
    Smart serial manager with a replace-queue for servo commands.
    
    Servo commands (s <ch> <angle>) use a per-channel slot — if a new value
    arrives before the old one is sent, the old one is silently replaced.
    This prevents Bluetooth buffer saturation when sliders are dragged fast.
    
    Non-servo commands (S, F, R, IMU_ON etc.) bypass the queue and are sent
    immediately — they are low-frequency and must not be dropped.
    """
    def __init__(self):
        self.ser     = None
        ...
        # Per-channel replace slots: {ch: angle_string} — only latest value kept
        self._servo_slots  = {}
    ...
    def disconnect(self):
        self._writer_alive = False
        self._slots_event.set()  # wake writer so it can exit
        with self.lock:
            if self.ser and self.ser.is_open:
                self.ser.close()
    ...
    def send(self, cmd: str):
        """
        Send a command. Servo commands go through the replace-queue;
        everything else is written immediately.
        """
        cmd = cmd.strip()
        # Servo commands: s <ch> <angle>
        if cmd.startswith("s "):
            parts = cmd.split()
            if len(parts) == 3:
                ch = parts[1]
                with self._slots_lock:
                    self._servo_slots[ch] = cmd   # replace any pending value
                self._slots_event.set()           # wake writer thread
                return
        # All other commands — send immediately
        self._write_now(cmd)

    def _write_loop(self):
        """
        Background writer thread — drains servo slots one per iteration.
        Sleeps until there's work, then sends one command and checks for more.
        Minimum gap between servo writes = serial_delay to avoid flooding.
        """
        while self._writer_alive:
            self._slots_event.wait()          # block until something queued
            ...
            while True:
                with self._slots_lock:
                    if not self._servo_slots:
                        break
                    # Pop one channel — dict preserves insertion order (Python 3.7+)
                    ch, cmd = next(iter(self._servo_slots.items()))
                    del self._servo_slots[ch]

                self._write_now(cmd)
                delay = SHARED_PARAMS.get("serial_delay", 0.05)
                time.sleep(delay)

                # If more slots waiting, keep draining; otherwise go back to sleep
                with self._slots_lock:
                    if not self._servo_slots:
                        break

    def _write_now(self, cmd: str):
        """Write directly to serial — bypasses queue."""
        with self.lock:
            if self.ser and self.ser.is_open:
                try:
                    self.ser.write((cmd + "\n").encode())
...
class L4SRController(tk.Tk):
    ...
    def _build_connection_bar(self):
        ...
        # Free/Resume — cuts PWM so servos go limp (support robot first!)
        tk.Label(bar, text=" |", bg=PANEL, fg=TEXT,
                 font=("Courier", 10)).pack(side="left", padx=(8, 4))
        self._style_btn(bar, "⚡ FREE",
                        lambda: self.serial_mgr.send("F"),
                        PURPLE).pack(side="left", padx=3)
        self._style_btn(bar, "↺ RESUME",
                        lambda: self.serial_mgr.send("R"),
    ...
    def _build_walk_tab(self, parent):
        ...
        # ── Balance stance ────────────────────────────────────────────────────
        # Static balance test — holds stance and applies IMU body_tilt correction
        # without any walking. Use this to verify IMU correction before gaits.
        bal_frame = tk.LabelFrame(parent, text="  ⚖  BALANCE STANCE  ",
                                  bg=PANEL, fg=GREEN,
                                  font=("Courier", 10, "bold"),
                                  bd=1, relief="groove")
        bal_frame.pack(padx=12, pady=(4, 8), fill="x")

        bal_top = tk.Frame(bal_frame, bg=PANEL)
        bal_top.pack(fill="x", padx=12, pady=(10, 4))

        tk.Label(bal_top,
                 text="Holds all legs at stance and applies live IMU tilt correction.\n"
                      "Enable IMU stream + auto body_tilt in the 📡 IMU tab first.",
                 font=("Courier", 8), bg=PANEL, fg="#557799",
                 justify="left").pack(side="left")

        bal_btn_row = tk.Frame(bal_frame, bg=PANEL)
        bal_btn_row.pack(pady=(4, 10))

        self._bal_status = tk.Label(bal_btn_row, text="⬛ OFF",
                                    font=("Courier", 11, "bold"),
                                    bg=PANEL, fg=YELLOW)
        self._bal_status.pack(side="left", padx=12)

        self._style_btn(bal_btn_row, "▶ START BALANCE",
                        self._balance_start, GREEN).pack(side="left", padx=4)
        self._style_btn(bal_btn_row, "■ STOP BALANCE",
                        self._balance_stop, HIGHLIGHT).pack(side="left", padx=4)
    ...
    def _build_imu_tab(self, parent):
        ...
        self._style_btn(ctrl, "▶ START STREAM", self._imu_stream_on,  GREEN).pack(side="left", padx=4)
        self._style_btn(ctrl, "■ STOP STREAM",  self._imu_stream_off, HIGHLIGHT).pack(side="left", padx=4)

        # ── Big angle readout ─────────────────────────────────────────────────
        angle_frame = tk.Frame(parent, bg=BG)
        angle_frame.pack(fill="x", padx=16, pady=12)
        for label, var, color in [
            ("ROLL  (angleX)", self._imu_angle_x, GREEN),
            ("PITCH (angleY)", self._imu_angle_y, ORANGE),
        ]:
            col = tk.Frame(angle_frame, bg=PANEL, padx=20, pady=12)
            col.pack(side="left", expand=True, fill="both", padx=8)
            tk.Label(col, text=label, font=("Courier", 10), bg=PANEL, fg=TEXT).pack()
            tk.Label(col, textvariable=var, font=("Courier", 32, "bold"),
                     bg=PANEL, fg=color).pack()
            tk.Label(col, text="degrees", font=("Courier", 9), bg=PANEL, fg=TEXT).pack()

        # ── Raw gyro / accel readout ──────────────────────────────────────────
        raw_frame = tk.Frame(parent, bg=BG)
        raw_frame.pack(fill="x", padx=16, pady=4)
        for section, items in [
            ("GYRO (°/s)", [("gx", self._imu_gx), ("gy", self._imu_gy), ("gz", self._imu_gz)]),
            ("ACCEL (g)",  [("ax", self._imu_ax), ("ay", self._imu_ay), ("az", self._imu_az)]),
        ]:
            sec = tk.Frame(raw_frame, bg=PANEL, padx=12, pady=10)
            sec.pack(side="left", expand=True, fill="both", padx=8)
            tk.Label(sec, text=section, font=("Courier", 10, "bold"),
                     bg=PANEL, fg=YELLOW).pack(anchor="w")
            for name, var in items:
                row = tk.Frame(sec, bg=PANEL)
                row.pack(fill="x", pady=2)
                tk.Label(row, text=f"{name}:", width=4, font=("Courier", 10),
        ...
        tilt_frame = tk.Frame(parent, bg=PANEL, padx=12, pady=10)
        tilt_frame.pack(fill="x", padx=16, pady=8)
        self._auto_tilt = tk.BooleanVar(value=False)
        tk.Checkbutton(tilt_frame,
                       text="  Auto body_tilt from IMU roll (angleX)",
                       variable=self._auto_tilt,
                       font=("Courier", 10), bg=PANEL, fg=TEXT,
                       selectcolor=ACCENT, activebackground=PANEL,
                       activeforeground=TEXT).pack(side="left")
        tk.Label(tilt_frame,
                 text="— maps roll → knee correction while walking  (scale: angleX × 0.3, clamped ±20°)",
    ...
    def _on_imu_data(self, line):
        """Called from serial read thread when a line starting with IMU: arrives."""
        try:
            parts = line[4:].split(",")   # strip "IMU:" prefix
            if len(parts) != 8:
                return
            angle_x, angle_y, gx, gy, gz, ax, ay, az = [float(p) for p in parts]
            def _update():
                self._imu_angle_x.set(round(angle_x, 1))
                self._imu_angle_y.set(round(angle_y, 1))
                self._imu_gx.set(round(gx, 1))
                self._imu_gy.set(round(gy, 1))
                self._imu_gz.set(round(gz, 1))
                self._imu_ax.set(round(ax, 3))
                self._imu_ay.set(round(ay, 3))
                self._imu_az.set(round(az, 3))
                # Auto body_tilt — maps roll to knee correction, clamped ±20°
